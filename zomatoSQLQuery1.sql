...
-- If buying each product generate points for ₹5 zomato point and each product have Different purchasing point 
...
SELECT p.product_id, p.product_name, COUNT(s.product_id) AS total_sold
FROM sales s
JOIN product p ON s.product_id = p.product_id
GROUP BY p.product_id, p.product_name
ORDER BY total_sold DESC;

-- Change the data type of product_name column to varchar
ALTER TABLE product
ALTER COLUMN product_name varchar(255); -- Adjust the length as needed
ALTER TABLE product
...
SELECT product_id, AVG(price / NULLIF(
  CASE
